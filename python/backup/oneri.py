...
# printleri log ile değiştirebilirsiniz
# şu anda bende gerekli php dosyaları olmadığı için deneyemiyorum
# benim yorum satırları belli, oraları dikkate alarak yapabilirsiniz.
# aşağıdaki kısımda diğer indikatör denemeleri için çekilen alanlar binancetan
# daha iyi performans için kaldıralabilir.
...
def brs(klines15m, klines3m, M=0, T=0, result={}):
    ...
    if result['date'] == klines15m[-1][0]:
        ...
        BRS = ((list(df15m['Close'])[-1] - (sum(df15m['Low']) / len(df15m['Low']))) / (
            (sum(df15m['High']) / len(df15m['High'])) - (sum(df15m['Low']) / len(df15m['Low'])))) * 100
        M = (2.5 / 3) * M + (0.5 / 3) * BRS
        T = (2.5 / 3) * T + (0.5 / 3) * M
        C = 3 * M - 2 * T
...
# burası benim ilklendirme alanım, size uygun şekilde buna benzer yapın
# bendeki veri yaısı buna benzerdi, sizdeki nasılsa onu yapın
BRS = {
    'date': 0,
    'BRS': 0,
    'M': 0.1848957,
                ...
                while klineConnect:
                    try:
                        ...
                        if ("Max retries exceeded" in str(e) or "Too many requests" in str(e) or "recvWindow" in str(e) or "Connection broken" in str(e) or "Please try again" in str(e)) and klineConnectCount < 3:
                            ...
                            raise Exception(e)
                # bu kısımda nereye kaydediyorsanız bu onceki m ve t değerlerini oraya kaydedin
