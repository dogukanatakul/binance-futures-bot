...
while True:
    ...
    while operationLoop:
        try:
            ...
            while klineConnect:
                try:
                    klines = client.futures_klines(symbol=getBot['parity'], interval=client.KLINE_INTERVAL_30MINUTE, limit=100)
                    klineConnect = False
                except Exception as e:
                    klineConnectCount += 1
                    ...
                    if getBot['status'] == 2 and orderStatus == False:
                        ...
                        if lastSide != getKDJ['side'] and firstTypeTrigger >= int(config('SETTING', 'FIRST_FAKE')) and fakeTrigger >= int(config('SETTING', 'FAKE_TRIGGER')):
                            ...
                            if not orderStatus:
                                ...
                                try:
                                    position = getPosition(client, getBot['parity'], lastType)
                                    klines = client.futures_klines(symbol=getBot['parity'], interval=client.KLINE_INTERVAL_15MINUTE, limit=100)
                                    # short / long
